...
import { AMERICANO_MUJERES_URL, SUMA_14_MUJERES_URL, tournamentData, SUMA_13_HOMBRES_URL, CURRENT_APP_VERSION, VERSION_KEY  } from "./contants-2.js";
...
function useVersionCheck() {
    useEffect(() => {
        ...
        if (storedVersion !== CURRENT_APP_VERSION) {
            console.log(`Versión local: ${storedVersion}. Versión actual: ${CURRENT_APP_VERSION}`);
            hardReload();
        } else {
            console.log(`Versión local (${storedVersion}) coincide con la versión actual.`)
        }
    }, []);
}
